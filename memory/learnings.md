...
- 2026-05-22 (EOD): **A fill is not complete until it is written to `trade_log.json`.** The GOOGL entry (5/20) existed only in `open_positions.md`. The `trades` array was empty and `weekly_trade_count` for the week read 0 — meaning the max-3-trades-per-week guardrail and any confidence-calibration math were silently running on stale, wrong data. The market-open execution routine that placed the order never logged it. Reconciled at this EOD. **Rule of thumb:** the buy order and the `trade_log.json` write must be a single atomic step in the execution routine — a position that exists at the broker but not in the trade log is a guardrail blind spot. Candidate hardening item for the weekly review.
- 2026-05-22 (EOD): **The infra/scheduler gap changed category from "annoying" to "risk-relevant" the moment capital was deployed.** Through Weeks 2–3 a day with no routine cost only a missed scan — zero positions, nothing at risk. 2026-05-21 produced no routines and no journal *while holding a live GOOGL position* — a full session of an unmonitored position with an unverified stop. Earlier "no trades, flat P&L" days hid a broken pipeline; now the broken pipeline hides unmanaged risk. The scheduler fix is no longer a convenience item.
...
- 2026-05-22 (EOD): First EOD with a live position. NVDA earnings (5/20 AMC) drove a strong post-print tech rally — SPY $733.80 → $745.28, QQQ to RSI 71.3 (overbought). GOOGL diverged DOWN from that tape (-1.31% open, below SMA 20) — the post-I/O "sell the news" reaction never reversed. A single-name catalyst (Google I/O) does not insulate a position from its own follow-through failure even while the index rips. MSFT half-trigger staled (stale-by 5/22 EOD, ADR-0004) — MACD cross never fired, histogram -0.56 → -0.36 but stayed negative; cleared without proposal, can re-arm fresh for June 2–3 Build.
