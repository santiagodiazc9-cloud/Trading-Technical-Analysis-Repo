...
- 2026-05-20 (EOD): **First fill of the program.** GOOGL 51 sh @ $387.07 (swing long, confidence 6/10, Google I/O catalyst). The order trail confirms the post-entry rule worked end-to-end: market buy filled 09:51 ET, 10% GTC trailing stop attached 15 seconds later. Position +0.38% at the close — but green on broad-market drift, not on the GOOGL-specific signal: the Stoch oversold bounce that justified the entry has NOT fired (Stoch K still 0.45). Rule of thumb confirmed: a same-day gain on an oversold-bounce swing is not thesis confirmation — verify the signal (Stoch turning up off the floor), not the P&L sign. The one indicator running against the thesis is MACD histogram, which deepened to -2.27 from -1.46 at proposal — the thing to watch on 5/21.
- 2026-05-20 (EOD): The first live data point lands in the WEAKEST proposable bucket (confidence 6 → bucket_5_6). Calibration tracking starts here — GOOGL's eventual outcome is bucket_5_6's first sample. A 5-6 confidence setup filling before any 7+ setup is rules-consistent (5-6 is the minimum proposable threshold) but means early calibration data will be noisy and low-bucket-weighted. Do not over-read one sample.
- 2026-05-20 (EOD): daytrade_count moved 0 → 1 on a day with no round-trip. Cause: Alpaca counts the same-day buy + GTC trailing-stop pair. Benign here (equity > $25k, PDT does not bind), but the lesson for sub-$25k logic: a position opened the same day with its protective stop attached can consume a PDT slot in Alpaca's count even though no day trade was made. Worth knowing before the rule ever binds.
...
- **AMZN-2026-05-15** (swing long, entry $264.00–$265.50, stop $260, target $278–$280, R:R 3.0:1, 74 shares, confidence 6/10): proposed 2026-05-15 pre-market. `Approved: NO` held through pre-market, market-open, midday, and EOD. EOD price $263.56 — $0.44 below the lower bound of the entry zone. Conditional execution gate not met any time during the session. **Stale-by date EOD 2026-05-19 pre-market (per ADR-0002, formalized in this weekly review).** Parameters held immutable per ADR-0003. **ARCHIVED 2026-05-19 midday — thesis broken.**
- **GOOGL-2026-05-20** (swing long, entry zone $387–391, stop $376, target $415, R:R 2.3:1, 51 shares, confidence 6/10): proposed 2026-05-20 pre-market, approved via Discord 13:28Z. **FILLED 2026-05-20 09:51 ET @ $387.07** (order 83bf9678), 10% GTC trailing stop attached. Setup lifecycle complete: proposed → approved → filled → now a managed position. Tracked in `memory/trade_log.json` and `memory/open_positions.md` Current Positions.
