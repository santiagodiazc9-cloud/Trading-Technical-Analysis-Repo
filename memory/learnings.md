...
## Pattern Notes
- 2026-05-18 (EOD): Market taking a measured pause after last week's overbought extension. **SPY MACD bearish cross confirmed at close** (histogram -0.06), **GOOGL also flashed MACD bearish cross** (histogram -0.23 at RSI 70.6). Two corroborating bearish crosses among Tech leaders — caution warranted but uptrend intact (SPY > SMA 20 $725). NVDA earnings binary tomorrow (5/19 AMC), no exposure (correct). MSFT half-trigger 1/2 held for 3rd session; MACD histogram narrowed -1.39 → -0.84 → -0.70 across 3 sessions, cross could fire any day this week. **AMZN closed back inside the entry zone** at $264.76 with Stoch K 0.79 — gate held, setup stales tomorrow per ADR-0002. **Microsoft Build catalyst date corrected** (June 2-3, not 5/19) — surfaces a new failure mode: catalyst entries in memory drift silently across sessions. 0 trades, full cash, patience working as designed.
- 2026-05-18 (EOD lesson): **Catalyst memory needs per-routine verification.** Microsoft Build = 5/19 persisted across several pre-market and EOD routines until today's correction caught it (actual: June 2-3). For any named conference / earnings / macro event used as a setup catalyst, the pre-market routine should re-verify the date that session rather than trust the memory line. Cheap to check; expensive to act on a phantom catalyst window.
- 2026-05-18 (EOD lesson): **Gate is most valuable when the setup gets prettier.** AMZN drifted back into the entry zone at the close with extreme oversold Stoch K (0.79). The exact intraday conditions where self-approval would feel "obviously right." `Approved: NO` held; ADR-0003 prevented zone mutation; ADR-0002 auto-stales the setup tomorrow regardless. Three lifecycle rules together produced the right outcome: no chase, clean retirement, fresh proposal required if conditions persist.
