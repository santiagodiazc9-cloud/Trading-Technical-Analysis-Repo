...
- 2026-05-21 (Midday Scan): **Partial-write data-integrity bug found.** The 2026-05-20 GOOGL execution updated `open_positions.md` (and placed the trailing stop) but never appended to `trade_log.json` — `trades` stayed `[]`, `total_trades` stayed 0, and `weekly_trade_count` stayed 0 despite a live $19.7k position. It also left the GOOGL-2026-05-20 block under a `## Pending Setups` heading with `Approved: YES`, which a later market-open routine could have re-executed. Both corrected at this midday scan. **Rule of thumb:** an execution routine must update every memory surface (`open_positions.md`, `trade_log.json`, `weekly_trade_count`) and demote the filled setup out of `## Pending Setups` in the SAME commit — a half-written fill silently corrupts the Dashboard and the risk math. Recommend a hard post-fill checklist in `routines/2_market_open_execution.md`.
