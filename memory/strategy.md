...
- 2026-05-18 (EOD): Candidate rule for Friday weekly review — **Per-routine catalyst verification.** Today's MSFT setup carried "Microsoft Build = 5/19" as a catalyst across multiple pre-market and EOD routines before this EOD caught that the conference is actually June 2-3. A named-catalyst entry (earnings date, conference date, macro release date) used in a setup or watchlist item must be re-verified by the pre-market routine each session rather than carried forward from memory. Cheap to check; expensive to act on a phantom catalyst window. Goal: add to Confluence Requirements item 6 (catalyst documented) → "catalyst documented AND verified-this-session." Not a hard change today; tabled for Friday 2026-05-22 weekly review.
