...
## Adjustments Log
- 2026-05-27 (EOD): No rule changes. Note for Friday weekly review: consider formalizing a "watch-zone breakdown protocol" — when a published watch zone breaks downward (price exits below the lower bound + SMA 20 lost), the next routine must re-validate the entry trigger and structural floor (SMA 50 for longs), not just re-price the bounds. Today's NVDA reset (zone $215-220 → $208-215, SMA 50 $198 as new structural floor) is the validation case. Defer formal ADR until at least one more case to confirm pattern.
