...
### 2026-06-05 16:46 UTC — Truncation regression on prior MCP push
- **Channel**: n/a (internal log)
- **Body**: The 16:42 UTC MCP push of this file mistakenly replaced the 21KB historical tail (5/25–6/04 archived entries) with stub `[archived — see prior commits for full content]` markers. Restoring the full 290-line content in this commit. Lesson: when using `mcp__github__push_files` as a substitute for `git push`, include the FULL current file content verbatim, not a hand-curated subset. Next routine should `git fetch origin && git reset --hard origin/main` to re-sync working tree to this restored state.
